...
function createRefreshTokenExpiry(now: Date): Date {
  return new Date(
    now.getTime() + REFRESH_TOKEN_TTL_DAYS * MILLISECONDS_PER_DAY,
  );
}

function mapRegisterPersistenceError(error: unknown): never {
  if (error instanceof UserRepositoryConflictError) {
    throw new AppError(
      ErrorCode.CONFLICT,
...
function mapUserRepositoryNotFoundError(error: unknown): never {
  if (error instanceof UserRepositoryNotFoundError) {
...
export class AuthService {
  ...
  async register(input: RegisterInput): Promise<RegisterResult> {
    ...
    try {
      ...
      mapRegisterPersistenceError(error);
    }
