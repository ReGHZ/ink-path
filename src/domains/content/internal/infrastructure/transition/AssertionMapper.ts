...
export const AssertionMapper = {
  ...
  // `narrative_transition_id ?? ""` and a cast of `operation` past a
  // three-member union. Both were written while the aggregate still modelled only
  // Phase 7's three declarable assertions, and both turned a legitimate row into a
  ...
  // `retract` row as "Invalid transition assertion type".
  //
  ...
  toDomain(row: PrismaAssertion): Assertion {
    const props: AssertionProperties = {
      id: row.id,
    ...
    return Assertion.reconstitute(props);
  },
  ...
  // an assertion's intent never changes, so there is no "last modified" to record.
  // Only `applied_at` moves, once.
  //
  // `appliedAt` IS written, `contentRevisionId` is not — and the asymmetry is
  // the step 4b change. The old rule ("every assertion is created pending, so a
  // create path able to set applied_at would let a caller declare an assertion as
  ...
  // `Assertion.assertFact()` — parentless, relationship shapes only — can
  ...
  toPersistence(
    ...
    return {
      ...
      // was a declared assertion (all four are null on that path) and is not
      ...
      targetOperation: snapshot.targetOperation,
  ...
  toUpdatePersistence(
    assertion: Assertion,
  ): Prisma.AssertionUncheckedUpdateManyInput {
    const snapshot = assertion.toSnapshot();
