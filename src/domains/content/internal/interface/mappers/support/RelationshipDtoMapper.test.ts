...
describe("RelationshipDtoMapper — perspective", () => {
  ...
  it("renders a relation type the registry no longer knows instead of throwing", () => {
    // `relation_type` is a plain string column with no CHECK constraint: a row
    // written by an older build must stay readable after the registry drops the
    // type. A 500 on GET would be a worse failure than a raw label.
    const row = detail({ relationType: "retired_type" });
