...
import { ASSERTION_LOG_EFFECT_TYPES } from "../../../domain/transition/Assertion.js";
...
// One row of `assertions` as the API shows it. Every nullable column is
...
// assertion carries the opposite, and the API does not pretend otherwise by
// splitting the response into two shapes. The client discriminates on
// `operation`, exactly as the domain does.
//
...
export const assertionResponseSchema = z
  .object({
    id: z.string(),
    ...
    // describes what a stored row IS, and `assertions` holds five
    ...
    operation: z.enum(ASSERTION_LOG_EFFECT_TYPES),
...
export type AssertionResponseDto = z.infer<
  typeof assertionResponseSchema
>;

// The transition ALWAYS travels with its assertions, in every response including
// the two lists, and that is not a convenience: `status` is derived from the
// assertions and never stored (`NarrativeTransition.ts:38-49`), so a response
// carrying the status without the assertions would be handing the client a
// conclusion it cannot check, and one that goes stale the moment any assertion is
...
// so trimming `assertions` from the wire would remove no query (notes §10, D11).
//
...
export const narrativeTransitionResponseSchema = z
  .object({
    ...
    assertions: z.array(assertionResponseSchema),
    createdAt: z.date(),
    updatedAt: z.date(),
  })
  .strict();
