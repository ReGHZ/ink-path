...
function createService() {
  ...
  return {
    ...
    relationships,
    locator,
    service: new WorldElementService(clock, idGenerator, worldElements, uow, locator),
  };
}
...
describe("WorldElementService", () => {
  ...
  describe("deleteWorldElement", () => {
    ...
    it("refuses the delete while a content relationship still points at the world element, and names the blocker", async () => {
      const {
        worldElements,
        contentRevisions,
        outboxEvents,
      ...
      expect(await worldElements.findById("we-1")).not.toBeNull();
